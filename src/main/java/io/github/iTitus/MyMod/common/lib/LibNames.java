package io.github.iTitus.MyMod.common.lib;

public class LibNames {

    public static final String AMMO_NAME = "ammo";

    public static final String BULLET_NAME = "bullet";

    public static final String DEBUG_NAME = "debug";

    public static final String DISPLAY_NAME = "display";
    public static final String DISPLAY_TE_NAME = "mymod:display_te";

    public static final String GUN_NAME = "gun";

    public static final String SPHERE_NAME = "sphere";
    public static final String SPHERE_TE_NAME = "mymod:sphere_te";

    public static final String TIMESHIFTER_NAME = "timeshifter";

    public static final String WHEEL_NAME = "wheel";
    public static final String WHEEL_TE_NAME = "mymod:wheel_te";

    public static final String ZERO_NAME = "zero";

}
